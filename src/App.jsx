...
const INITIAL_INSUMOS = [
  { id:"IMA-001", descripcion:"Imán flexible 10mm", unidad:"metro", stock:250, minimo:50, maximo:500, proveedor:"Magnoimport SA", esIman:true },
  { id:"IMA-002", descripcion:"Imán flexible 15mm", unidad:"metro", stock:180, minimo:40, maximo:400, proveedor:"Magnoimport SA", esIman:true },
  { id:"IMA-003", descripcion:"Imán flexible 20mm", unidad:"metro", stock:90,  minimo:30, maximo:300, proveedor:"Magnoimport SA", esIman:true },
  { id:"PER-001", descripcion:"Perfil PVC gris 8mm",   unidad:"metro", stock:320, minimo:80, maximo:600, proveedor:"PlásticosSur", esPerfil:true },
  { id:"PER-002", descripcion:"Perfil PVC negro 10mm",  unidad:"metro", stock:210, minimo:60, maximo:500, proveedor:"PlásticosSur", esPerfil:true },
  { id:"PER-003", descripcion:"Perfil PVC blanco 12mm", unidad:"metro", stock:45,  minimo:60, maximo:400, proveedor:"PlásticosSur", esPerfil:true },
  { id:"MAN-001", descripcion:"Manguera PVC 8mm",  unidad:"metro", stock:80, minimo:20, maximo:200, proveedor:"PlásticosSur", esManguera:true },
  { id:"MAN-002", descripcion:"Manguera PVC 10mm", unidad:"metro", stock:60, minimo:15, maximo:150, proveedor:"PlásticosSur", esManguera:true },
...
const INITIAL_CLIENTES = [
  { id:"CLI-001", nombre:"Servicio Técnico Martínez", telefono:"11-4523-9900", direccion:"Av. Corrientes 1234, CABA", transporte:"Andreani" },
  { id:"CLI-002", nombre:"Frigorífico El Sur",         telefono:"11-3345-7711", direccion:"Ruta 3 km 42, Lomas de Zamora", transporte:"OCA" },
  { id:"CLI-003", nombre:"Reparaciones López",         telefono:"11-6677-2200", direccion:"Mitre 550, Quilmes", transporte:"Retira en local" },
];

const INITIAL_PEDIDOS = [
  { id:"PED-001", fecha:"12/06/2026", clienteId:"CLI-001", cliente:"Servicio Técnico Martínez", telefono:"11-4523-9900", transporte:"Andreani", via:"Teléfono", estado:"entregado", obs:"",
    items:[{ id:1, tipoProducto:"Marco", aplicacion:"Heladera", perfilId:"PER-001", imanId:"IMA-001", ancho:600, alto:800, largo:null, cantidad:2, descripcion:"Marco Heladera 600x800mm" }] },
  { id:"PED-002", fecha:"16/06/2026", clienteId:"CLI-002", cliente:"Frigorífico El Sur", telefono:"11-3345-7711", transporte:"OCA", via:"Mail", estado:"pendiente", obs:"Para el viernes",
    items:[
      { id:1, tipoProducto:"Marco", aplicacion:"Cámara frigorífica", perfilId:"PER-002", imanId:"IMA-002", ancho:1000, alto:2000, largo:null, cantidad:1, descripcion:"Marco Cámara 1000x2000mm" },
      { id:2, tipoProducto:"Manguera", aplicacion:null, perfilId:"MAN-001", imanId:null, ancho:null, alto:null, largo:3000, cantidad:2, descripcion:"Manguera PVC 8mm - 3000mm" },
    ]},
  { id:"PED-003", fecha:"18/06/2026", clienteId:"CLI-003", cliente:"Reparaciones López", telefono:"11-6677-2200", transporte:"Retira en local", via:"Teléfono", estado:"en fabricacion", obs:"Urgente",
    items:[{ id:1, tipoProducto:"Tira", aplicacion:"Freezer", perfilId:"PER-001", imanId:"IMA-001", ancho:null, alto:null, largo:2500, cantidad:3, descripcion:"Tira Freezer 2500mm" }] },
];
...
export default function App() {
  ...
  const [chat, setChat]       = useState([{ role:"assistant", text:"¡Hola Valentino! Soy tu asistente. Preguntame sobre stock, pedidos o insumos." }]);
  ...
  function imprimirPedido(pedido) {
    ...
    win.document.write(`
      ...
      <style>
        ...
        .blk-title { font-size: 11px; font-weight: 700; color: #6b7280; text-transform: uppercase; margin-bottom: 6px; }
        .blk-val { font-size: 14px; font-weight: 600; }
      ...
      <div class="grid">
        <div class="blk"><div class="blk-title">Cliente</div><div class="blk-val">${pedido.cliente}</div>${pedido.telefono ? `<div style="font-size:13px;color:#6b7280;margin-top:4px">📞 ${pedido.telefono}</div>` : ""}</div>
        <div class="blk"><div class="blk-title">Transporte</div><div class="blk-val">${pedido.transporte || "—"}</div></div>
      </div>
      ${pedido.obs ? `<div class="blk" style="margin-bottom:24px"><div class="blk-title">Observaciones</div><div class="blk-val">${pedido.obs}</div></div>` : ""}
      <table>
        <tr><th>Producto</th><th>Medidas</th><th>Material</th><th>Cant.</th></tr>
        ${pedido.items.map(item => {
          const medida = item.ancho ? `${item.ancho}×${item.alto}mm` : item.largo ? `${item.largo}mm` : "—";
          const mat = calcularMateriales(item);
          const matStr = mat ? `Perfil: ${mmToM(mat.perfilMm*item.cantidad)}m${mat.imanMm>0?` / Imán: ${mmToM(mat.imanMm*item.cantidad)}m`:""}` : "—";
          return `<tr><td>${item.tipoProducto}${item.aplicacion?" — "+item.aplicacion:""}</td><td>${medida}</td><td>${matStr}</td><td>${item.cantidad}</td></tr>`;
        }).join("")}
      </table>
      <div class="footer">Generado por BurleteStock · ${today()}</div>
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model:"claude-sonnet-4-6", max_tokens:1000,
          system:`Asistente de stock para empresa argentina de burletes (marcos, tiras, ángulos, mangueras). Dueño: Valentino. Español rioplatense, breve.\n${ctx}`,
          messages:[...chat.slice(1).map(m=>({role:m.role==="assistant"?"assistant":"user",content:m.text})),{role:"user",content:msg}]
        })
      });
    ...
    <div style={{ fontFamily:"sans-serif", background:"#f0f4f1", height:"100vh", maxWidth:1200, margin:"0 auto", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1a5c2e 0%,#2d8a4e 100%)", padding:"12px 16px", color:"white", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", gap:10, maxWidth:1200, margin:"0 auto" }}>
          ...
          {(alertasInsumos.length+pedidosPendientes.length)>0 && (
            <div style={{ background:"#ef4444", borderRadius:20, padding:"3px 10px", fontSize:12, fontWeight:700 }}>
              {alertasInsumos.length+pedidosPendientes.length} alertas
            </div>
          )}
        </div>
      </div>
            ...
            <div style={{ flex:1, padding:"10px 8px", overflowY:"auto" }}>
              {tabs.map(t=>(
                <button key={t.id} onClick={()=>{setTab(t.id);setMenuOpen(false);}} style={{
                  ...
                  border:"none", borderRadius:10, cursor:"pointer", marginBottom:2, textAlign:"left",
                  background: tab===t.id ? "#f0fdf4":"transparent",
                  color: tab===t.id ? "#1a5c2e":"#374151",
                  fontWeight: tab===t.id ? 700:400, fontSize:15,
                }}>
        ...
        <div style={{ padding:"16px", maxWidth:800, margin:"0 auto" }}>
            ...
            <div>
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:16 }}>
                {[
                  ...
                  { label:"Clientes",           value:clientes.length,           color:"#8b5cf6", icon:"👥", action:()=>setTab("clientes") },
                  { label:"Pedidos entregados", value:pedidos.filter(p=>p.estado==="entregado").length, color:"#10b981", icon:"✅" },
                ].map((c,i)=>(
          ...
          {tab==="pedidos" && (
            <div>
                    ...
                    {items.map(pedido=>(
                      <div key={pedido.id} style={{ ...G.card, borderLeft:`4px solid ${cfg.border}` }}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
                          ...
                          <button onClick={()=>imprimirPedido(pedido)} title="Imprimir" style={{ background:"#f3f4f6", border:"none", borderRadius:8, padding:"6px 10px", cursor:"pointer", fontSize:16 }}>🖨️</button>
                        </div>
                        {pedido.items.map((item,i)=>{
                          ...
                          return (
                            <div key={i} style={{ background:"#f9fafb", borderRadius:8, padding:"8px 10px", marginBottom:4 }}>
                              ...
                              {mat && <div style={{ fontSize:11, color:"#6b7280", marginTop:2 }}>Perfil: {mmToM(mat.perfilMm*item.cantidad)}m{mat.imanMm>0?` · Imán: ${mmToM(mat.imanMm*item.cantidad)}m`:""}</div>}
                            </div>
                          );
                        })}
                        ...
                        {estado!=="entregado" && (
                          <div style={{ display:"flex", gap:6, marginTop:10, flexWrap:"wrap" }}>
                            ...
                            {estado==="pendiente" && <button onClick={()=>cambiarEstado(pedido.id,"listo")} style={{ background:"#f0fdf4", border:"1px solid #86efac", borderRadius:8, padding:"6px 12px", fontSize:12, color:"#166534", fontWeight:600, cursor:"pointer" }}>✅ Ya está listo</button>}
              ...
              {pedidos.length===0 && <div style={{ textAlign:"center", padding:40, color:"#9ca3af" }}><div style={{ fontSize:40 }}>📭</div><div style={{ marginTop:8 }}>Sin pedidos</div></div>}
            </div>
          )}

          {/* ── CLIENTES ── */}
          {tab==="clientes" && (
            <ClientesTab clientes={clientes} setClientes={setClientes} onNuevoPedido={(c)=>setModal({tipo:"nuevoPedido", clientePrefill:c})} />
          )}
                    ...
                    return (
                      <div key={item.id} style={G.card}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                          <div><div style={{ fontWeight:700, fontSize:13 }}>{item.descripcion}</div><div style={{ fontSize:11, color:"#9ca3af" }}>{item.id} · {item.proveedor}</div></div>
          ...
          {tab==="ia" && (
            <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 120px)" }}>
              ...
              <div style={{ flex:1, overflowY:"auto", display:"flex", flexDirection:"column", gap:10, paddingBottom:10 }}>
                {chat.map((m,i)=>(
                  <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
                    <div style={{ maxWidth:"85%", padding:"10px 14px", borderRadius:m.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", background:m.role==="user"?"#1a5c2e":"white", color:m.role==="user"?"white":"#111", fontSize:14, lineHeight:1.5, boxShadow:"0 1px 4px rgba(0,0,0,0.08)" }}>
                      {m.text}
                    </div>
                  </div>
                ))}
              ...
              <div style={{ marginTop:8, display:"flex", gap:6, flexWrap:"wrap" }}>
                {["¿Qué reponer?","¿Cuánto imán queda?","¿Pedidos pendientes?"].map(q=>(
          ...
          {/* ── CONFIG ── */}
          {tab==="config" && (
            <ConfigInsumos insumos={insumos} onUpdateInsumo={(id,ch)=>setInsumos(prev=>prev.map(i=>i.id===id?{...i,...ch}:i))} onAddInsumo={item=>setInsumos(prev=>[...prev,item])} onDeleteInsumo={id=>setInsumos(prev=>prev.filter(i=>i.id!==id))} />
          )}
      ...
      {/* MODALES */}
      {modal?.tipo==="nuevoPedido" && <NuevoPedidoModal insumos={insumos} clientes={clientes} clientePrefill={modal.clientePrefill} onConfirm={agregarPedido} onClose={()=>setModal(null)}/>}
...
function ClientesTab({ clientes, setClientes, onNuevoPedido }) {
  ...
  function guardar() {
    ...
    if (editando.nuevo) {
      setClientes(prev=>[...prev,{...form,id:`CLI-${String(Date.now()).slice(-4)}`}]);
    } else {
  ...
  function eliminar(id) { setClientes(prev=>prev.filter(c=>c.id!==id)); }
  ...
  if (editando) return (
    <div>
      ...
      <div style={G.card}>
        ...
        {[["nombre","Nombre *","text"],["telefono","Teléfono","text"],["direccion","Dirección","text"]].map(([key,label])=>(
          <div key={key}>
            <label style={G.lbl}>{label}</label>
            <input style={G.inp} value={form[key]||""} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} />
          </div>
        ))}
        <label style={G.lbl}>Transporte habitual</label>
        <input style={G.inp} value={form.transporte||""} onChange={e=>setForm(f=>({...f,transporte:e.target.value}))} placeholder="ej: Andreani, OCA, Retira en local..." list="transportes-list"/>
        <datalist id="transportes-list">{TRANSPORTES.map(t=><option key={t} value={t}/>)}</datalist>
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        ...
        <button onClick={()=>{setEditando({nuevo:true});setForm({id:"",nombre:"",telefono:"",direccion:"",transporte:""});}} style={{ background:"#1a5c2e", color:"white", border:"none", borderRadius:8, padding:"8px 14px", fontSize:13, cursor:"pointer", fontWeight:600 }}>+ Nuevo</button>
      </div>
        ...
        <div key={c.id} style={G.card}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            ...
            <div style={{ display:"flex", gap:6, marginLeft:10, flexShrink:0 }}>
              ...
              <button onClick={()=>eliminar(c.id)} style={{ background:"#fff1f2", border:"1px solid #fecdd3", borderRadius:8, padding:"6px 10px", fontSize:13, color:"#be123c", cursor:"pointer" }}>🗑️</button>
            </div>
          </div>
        </div>
      ))}
...
// ── NUEVO PEDIDO MODAL ────────────────────────────────────
function NuevoPedidoModal({ insumos, clientes, clientePrefill, onConfirm, onClose }) {
  ...
  function newItem() { return { id:Date.now(), tipoProducto:"Marco", aplicacion:"Heladera", perfilId:"", imanId:"", ancho:"", alto:"", largo:"", cantidad:1 }; }

  function buildDesc(item) {
    if (["Marco","Burlete","Angulo"].includes(item.tipoProducto)) return `${item.tipoProducto} ${item.aplicacion||""} ${item.ancho}×${item.alto}mm`;
  ...
  function selCliente(id) {
    ...
    if (!c) return;
    setClienteId(id); setCliente(c.nombre); setTelefono(c.telefono||""); setTransporte(c.transporte||"");
  }

  const perfiles = insumos.filter(i=>i.esPerfil||i.esManguera);
  const imanes   = insumos.filter(i=>i.esIman);
  const inp = G.inp; const lbl = G.lbl;
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.5)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300 }}>
      <div style={{ background:"white", borderRadius:"20px 20px 0 0", padding:20, width:"100%", maxWidth:600, maxHeight:"92vh", overflowY:"auto" }}>
        ...
        <select style={inp} value={clienteId} onChange={e=>selCliente(e.target.value)}>
          <option value="">— Seleccionar cliente —</option>
          {clientes.map(c=><option key={c.id} value={c.id}>{c.nombre}</option>)}
        </select>
        {!clienteId && <>
          <label style={lbl}>Nombre del cliente *</label>
          <input style={inp} value={cliente} onChange={e=>setCliente(e.target.value)} placeholder="Nombre"/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          <div>
            ...
            <input style={inp} value={transporte} onChange={e=>setTransporte(e.target.value)} placeholder="Andreani, OCA..." list="tlist"/>
          ...
          <div key={item.id} style={{ background:"#f9fafb", borderRadius:10, padding:12, marginBottom:10, border:"1px solid #e5e7eb" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              <div><label style={lbl}>Tipo</label>
                <select style={inp} value={item.tipoProducto} onChange={e=>upd(idx,"tipoProducto",e.target.value)}>
                  {TIPOS.map(t=><option key={t}>{t}</option>)}
                </select>
              </div>
              {item.tipoProducto!=="Manguera" && <div><label style={lbl}>Aplicación</label>
                <select style={inp} value={item.aplicacion} onChange={e=>upd(idx,"aplicacion",e.target.value)}>
            ...
            {["Marco","Burlete","Angulo"].includes(item.tipoProducto) && (
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                <div><label style={lbl}>Ancho (mm)</label><input type="number" style={inp} value={item.ancho} onChange={e=>upd(idx,"ancho",e.target.value)} placeholder="ej: 600"/></div>
                <div><label style={lbl}>Alto (mm)</label><input type="number" style={inp} value={item.alto} onChange={e=>upd(idx,"alto",e.target.value)} placeholder="ej: 800"/></div>
              </div>
            )}
            {["Tira","Manguera"].includes(item.tipoProducto) && (
              <div><label style={lbl}>Largo (mm)</label><input type="number" style={inp} value={item.largo} onChange={e=>upd(idx,"largo",e.target.value)} placeholder="ej: 2500"/></div>
            )}
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
              <div><label style={lbl}>Perfil / Material</label>
                <select style={inp} value={item.perfilId} onChange={e=>upd(idx,"perfilId",e.target.value)}>
                  <option value="">Seleccionar</option>
                  {perfiles.map(p=><option key={p.id} value={p.id}>{p.descripcion}</option>)}
                </select>
              </div>
              {item.tipoProducto!=="Manguera" && <div><label style={lbl}>Imán</label>
                <select style={inp} value={item.imanId} onChange={e=>upd(idx,"imanId",e.target.value)}>
                  <option value="">Seleccionar</option>
                  {imanes.map(p=><option key={p.id} value={p.id}>{p.descripcion}</option>)}
                </select>
              </div>}
            </div>
            <div><label style={lbl}>Cantidad</label><input type="number" min={1} style={inp} value={item.cantidad} onChange={e=>upd(idx,"cantidad",e.target.value)}/></div>
            {(()=>{
              const mat = calcularMateriales(item);
              if (!mat) return null;
              return <div style={{ background:"#f0fdf4", borderRadius:8, padding:"6px 10px", fontSize:12, color:"#166534" }}>📐 Usará: <b>{mmToM(mat.perfilMm*item.cantidad)}m</b> perfil{mat.imanMm>0?<> + <b>{mmToM(mat.imanMm*item.cantidad)}m</b> imán</>:""}</div>;
            })()}
          </div>
        ))}
        ...
        <input style={inp} value={obs} onChange={e=>setObs(e.target.value)} placeholder="Urgencia, fecha de entrega..."/>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginTop:6 }}>
          ...
          <button onClick={()=>{ if(!(cliente||clienteId)||!items.length) return; onConfirm({clienteId,cliente:cliente||clientes.find(c=>c.id===clienteId)?.nombre,telefono,transporte,via,obs,items:items.map(it=>({...it,descripcion:buildDesc(it)}))});}} style={{ padding:"12px", background:"#1a5c2e", color:"white", border:"none", borderRadius:10, fontWeight:700, cursor:"pointer", fontSize:14 }}>Guardar pedido</button>
        </div>
      </div>
    </div>
  );
